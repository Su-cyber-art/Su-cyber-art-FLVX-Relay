...
func sendBatchTrafficReport(ctx context.Context, reportItems []TrafficReportItem) (bool, error) {
	...
	return postJSONWithFallback(ctx, strings.Split(httpReportURL, ","), requestBody, "GOST-Traffic-Reporter/1.0", 5*time.Second, &preferredUploadURL)
}
...
func sendConfigReport(ctx context.Context) (bool, error) {
	...
	return postJSONWithFallback(ctx, strings.Split(configReportURL, ","), requestBody, "Config-Reporter/1.0", 10*time.Second, &preferredConfigURL)
}
