...
const (
	defaultMetricsRangeMs = int64(60 * 60 * 1000)
	maxMetricsRangeMs     = int64(24 * 60 * 60 * 1000)
)
...
func (h *Handler) monitorNodeMetricsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasSuffix(rest, "/metrics/latest") {
		h.handleNodeMetricsLatest(w, strings.TrimSuffix(rest, "/metrics/latest"))
		return
	}
...
func (h *Handler) monitorNodeListHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range nodes {
		...
		items = append(items, monitorNodeListItem{ID: n.ID, Inx: n.Inx, Name: n.Name, Status: n.Status, UpdatedTime: updated})
	}
...
func (h *Handler) monitorTunnelListHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, t := range tunnels {
		items = append(items, monitorTunnelListItem{ID: t.ID, Inx: t.Inx, Name: t.Name, Status: t.Status, UpdatedTime: t.UpdatedTime})
	}
	response.WriteJSON(w, response.OK(items))
}
...
func (h *Handler) handleNodeMetrics(w http.ResponseWriter, r *http.Request, nodeIDStr string) {
	...
	if startMs <= 0 || endMs <= 0 || endMs < startMs || endMs-startMs > maxMetricsRangeMs {
...
func (h *Handler) handleNodeMetricsLatest(w http.ResponseWriter, nodeIDStr string) {
	nodeID, err := strconv.ParseInt(nodeIDStr, 10, 64)
	if err != nil || nodeID <= 0 {
		response.WriteJSON(w, response.ErrDefault("无效的节点ID"))
		return
...
func (h *Handler) monitorServiceListHandler(w http.ResponseWriter, r *http.Request) {
	...
	items, err := h.repo.ListServiceMonitors()
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
...
func (h *Handler) monitorServiceCreate(w http.ResponseWriter, r *http.Request) {
	...
	m := &model.ServiceMonitor{Name: name, Type: monitorType, Target: target, IntervalSec: intervalSec, TimeoutSec: timeoutSec, NodeID: req.NodeID, Enabled: enabled, CreatedTime: now, UpdatedTime: now}
...
func (h *Handler) monitorServiceUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if v := strings.ToLower(strings.TrimSpace(req.Type)); v == "tcp" || v == "icmp" {
		existing.Type = v
	}
