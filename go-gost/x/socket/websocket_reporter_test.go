...
func TestBuildWebSocketCandidatesSecureFirst(t *testing.T) {
	...
	if !strings.HasPrefix(candidates[0], "wss://") || !strings.HasPrefix(candidates[1], "ws://") {
...
func TestBuildWebSocketCandidatesUsesPreferredScheme(t *testing.T) {
	...
	if !strings.HasPrefix(candidates[0], "ws://") || !strings.HasPrefix(candidates[1], "wss://") {
		t.Fatalf("unexpected candidates: %#v", candidates)
	}
}
...
func TestBuildWebSocketCandidatesNormalizesSchemePrefixedAddr(t *testing.T) {
	...
	if !strings.HasPrefix(candidates[0], "wss://panel.example.com:443/") || !strings.HasPrefix(candidates[1], "ws://panel.example.com:443/") {
...
func TestDialWebSocketWithFallbackTriesWSAfterWSSFailure(t *testing.T) {
	...
	_, usedURL, err := dialWebSocketWithFallback(&websocket.Dialer{}, []string{"wss://panel.example.com/system-info?type=1&secret=abc", "ws://panel.example.com/system-info?type=1&secret=abc"})
	if err != nil {
		t.Fatalf("expected fallback success, got err=%v", err)
	}
	if !strings.HasPrefix(usedURL, "ws://") {
		t.Fatalf("expected ws fallback url, got %s", usedURL)
	}
	if len(attempts) != 2 || !strings.HasPrefix(attempts[0], "wss://") || !strings.HasPrefix(attempts[1], "ws://") {
